...
export default function DashboardPage() {
  const {
    summary, transactions, fetchSummary, fetchTransactions,
    fetchAccounts,
  } = useExpenseStore();

  const {
    quotes, fetchLiveQuotes,
  ...
  const totalPortfolio = quotes.reduce((s, q) => s + (q.marketValue ?? q.costBasis), 0);
  ...
  const networthChartData = useMemo(() => {
    ...
    networthHistory.forEach(h => { snapshotLookup[h.month] = h.investmentsValue; });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="stat-card border-accent/30 bg-accent/5">
          ...
          <span className="stat-value text-accent-light">{formatCurrency(totalNetWorth)}</span>
        ...
        <div className="stat-card">
          ...
          <span className={`text-xs font-mono ${netPL >= 0 ? 'text-green' : 'text-red'}`}>
        ...
        <div className="card lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={networthChartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
              ...
              />
              <Legend
                formatter={v => <span style={{ color: '#9090b8', fontSize: 11 }}>{v}</span>}
              />
              {/* Net Worth on top as the primary line */}
              <Area type="monotone" dataKey="Net Worth" stroke="#7c6aff" strokeWidth={2.5}
                fill="url(#nwGrad)"
                dot={{ fill: '#7c6aff', r: 3, stroke: '#1a1a26', strokeWidth: 2 }}
                activeDot={{ r: 5 }}
              />
              {/* Cash and Portfolio as secondary reference lines */}
              <Area type="monotone" dataKey="Cash" stroke="#34d399" strokeWidth={1.5}
                fill="url(#cashGrad)" strokeDasharray="4 3"
                dot={{ fill: '#34d399', r: 2 }} activeDot={{ r: 4 }}
              />
              <Area type="monotone" dataKey="Portfolio" stroke="#f5c542" strokeWidth={1.5}
                fill="url(#portGrad)" strokeDasharray="4 3"
                dot={{ fill: '#f5c542', r: 2 }} activeDot={{ r: 4 }}
              />
            </AreaChart>
          </ResponsiveContainer>
          <p className="text-xs text-text-muted mt-1 text-center">
            Portfolio uses live price for current month · past months use saved snapshots from the Portfolio page
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/money-flow" className="text-xs text-accent hover:text-accent-light transition-colors">
              View all →
            </Link>
          </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/networth" className="text-xs text-accent hover:text-accent-light transition-colors">
              View all →
            </Link>
          </div>
          {quotes.length > 0 ? (
